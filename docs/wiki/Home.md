# Ramses Extras Wiki

This wiki is the **in-repo** documentation for the `ramses_extras` Home Assistant integration.

- Source of truth: `ramses_extras/docs/wiki/`
- Intended destination: `ramses_extras.wiki` (GitHub Wiki)

## Where to start
- Users:
  - [Getting Started (Users)](Getting-Started-Users.md)
- Developers:
  - [Getting Started (Developers)](Getting-Started-Devs.md)
- High-level orientation:
  - [Overview](Overview.md)

## Pages
- [Overview](Overview.md)
- [Getting Started (Users)](Getting-Started-Users.md)
- [Getting Started (Developers)](Getting-Started-Devs.md)

## Architecture
- [System Architecture](System-Architecture.md)
- [Feature System](Feature-System.md)
- [Framework Foundation](Framework-Foundation.md)
- [Device Feature Management](Device-Feature-Management.md)
- [Entity Management](Entity-Management.md)
- [Home Assistant Integration](Home-Assistant-Integration.md)
- [Frontend Architecture](Frontend-Architecture.md)

## Reference
- [Development Guide](Development-Guide.md)
- [Debugging and Troubleshooting](Debugging-and-Troubleshooting.md)
- [API Reference](API-Reference.md)
- [Implementation Details](Implementation-Details.md)
