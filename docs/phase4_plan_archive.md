...
> **Aug 14 2026 update:** ramses_rf Phase 6 CLOSED (issue 1001) — PRs
> 11+12 merged (dataclass payload cutover + legacy parser purge).
> `payload_to_dict()` boundary adapter retained so ramses_cc still
> receives dict payloads. Issue 1027 (MessageStore state restoration)
> raised and answered by PWhite-Eng — MessageStore approach rejected
> (4 technical blockers: async hydration race, ghost device
> resurrection, double-ingestion, in-memory environments). PR 953
> opened: SAVE_STATE_INTERVAL 5m→30m (PWhite-Eng's short-term
> recommendation). Long-term fix is Phase 9 CQRS snapshots. Step 8
> added (remove obsolete `hvac_schema` cache, after release feedback).
...
| Aug 12 2026 | ramses_rf PR 1031 merged (Phase 6 PR 11) | Active dataclass payload decoders activated. `payload_to_dict()` boundary adapter retained for downstream compat. |
| Aug 12 2026 | ramses_cc PRs 942, 944, 946 merged | Bug fixes: discovered device comparison by id (not identity), authoritative domain_id for BDR placement (issue 931), backfill 'belongs to' comments from schema `_bound` trait. |
| Aug 13 2026 | ramses_rf PR 1034 merged (Phase 6 PR 12) | Final pipeline cutover — legacy regex parsers purged, full dataclass payload decoding. Phase 6 (issue 1001) CLOSED. |
| Aug 13 2026 | ramses_rf PR 1037 merged | Polymorphic payload dispatchers for 18 opcodes with variable-length payloads. Post-Phase 6 refinement. |
| Aug 13 2026 | ramses_cc PRs 949, 950, 951 merged | Issue 947 fixes: zone class inference, orphaned BDR placement, zone name mismatch notification, zone device name refresh in sync_topology. |
| Aug 13 2026 | ramses_rf PRs 1035, 1036, 1038 merged | Bug fixes: per-zone 0004 polling restored (issue 947), DHW sensor heartbeat timeout (issue 925), multi-room zone temp routing (issue 948). |
...
| Aug 19 2026 | Step 6e REM→FAN DONE (PR 1091, draft) | REM/CO2 sending directed W/RQ/I to FAN (32:) now sets `bound_to` in discovery_scan.py. More authoritative than FAN→REM — REM only sends to its 1FC9-paired FAN. Directed I is proof (broadcast I filtered by `_is_valid_address`). 3 locations (known/new/existing device), 4 new unit tests, all 2588 ramses_rf tests pass. |
| Aug 19 2026 | Step 7 assessed — DEFERRED INDEFINITELY | `StateUpdatedEvent` exists internally but no external subscription API on Gateway. Issue 809 has no activity. Phase 639 blueprint doesn't mention it. Nobody planning to build it. `asyncio.sleep(0)` interim works (all tests pass). |
| Aug 19 2026 | Step 4 assessed — wiki documentation needed | Wiki page `2.1-Configuration-step-3:-Schemas.md` missing `_polling_interval`, `_is_battery`, `_locked`, `_skipped` traits. Native CQRS builders for 22F1/22F7/22B0/2411/31DA not mentioned (users can drop `_commands` for these codes). |
| Aug 19 2026 | Step 6e REM→FAN **verified on hass** | CO2 `37:126776` detected as `bound_to=32:153289` from directed `I 31E0` packets after `sudo make all` restart. Placed in FAN's `sensors[]` list, removed from `orphans_hvac`. Initial deployment had stale code (scan engine started before commit landed — Python doesn't hot-reload). PR 1091 rebased on latest master (includes PR 1089 FAN→DIS reclassification + PR 1090 MQTT HGI warning). |
| Aug 19 2026 | Issue 1000 PR 1004 MERGED — FAN→DIS contradiction detection | Scan engine re-classifies known devices after 3+ evidence-based contradictions (VC pair matches, CTL-only codes). Prefix fallbacks (e.g. 37:→REM) do NOT count as contradictions — prevents CO2 being re-classified as REM just because 37: falls to REM by prefix. ramses_cc `check_class_mismatches` skips HVAC devices with low/medium confidence; `_check_rf_contradictions` reads from ramses_rf's known_list instead. |
| Aug 19 2026 | Issue 1002 PR 1090 MERGED — MQTT gateway HGI warning | `_set_active_hgi(None)` defers known_list check for MQTT bridges (ramses_esp) until transport learns real HGI ID from "online" LWT message. |
| Aug 19 2026 | Issue 1003 PR 1005 MERGED — foreign-owner NEW flag | `_extract_schema_device_ids` now includes foreign devices so they're not flagged as NEW on every scan cycle. Separated from `_derive_known_list_from_schema` (which still excludes foreign devices for ramses_rf's known_list). |
| Aug 19 2026 | PR 1006 (draft) — foreign-owner discovery exclusion | Complementary to PR 1005: passes `foreign_device_ids` to `sync_with_schema` and `check_for_new_devices` so foreign-owner devices are marked REMOVED (not visible in discovery UI at all). **Verified on hass**: neighbour's FAN `37:154519` (`_owner: not-me`) now `status=removed`. 1360 ramses_cc tests pass. |
