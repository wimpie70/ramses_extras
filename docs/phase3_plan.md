...
**Created:** Jul 2026
**Status:** Phase 2.5 DONE (PR 810) — Phase 3a implemented (PR 811), awaiting review — Phase 3b in design
...
| **Status** | 3a DONE (PR 811), 3b in design | Not started (Phase 2.95 merged) |
| **Depends on** | 3a: strip workaround (DONE). 3b: ramses_rf PR (list `bound` + strip+map pipeline) | Phase 2 complete (SQLite + RAM cache) |
| **Blocks?** | No — 3a shipped independently, 3b needs small ramses_rf PR | No — can ship without ramses_cc's `_commands` |
...
- **Phase 3b (design):** `_commands` moves to FAN entries,
  `{verb, code, payload}` dicts. `_bound` accepts `list[str]`.
  Strip+map pipeline moves to ramses_rf. See
  `phase3b_fan_commands_design.md`.

**Key insight:** ramses_cc's Phase 3a shipped using the same
`_strip_schema_extensions` workaround as Phase 2. Phase 3b moves the
strip+map logic to ramses_rf (so the CLI also benefits). ramses_rf's
issue 530 Phase 3 is a larger refactor that will eventually shrink
`_commands` (22F7/22B0 get native builders) but is NOT a dependency.
...
**Phase 3b (design):** Commands move to FAN schema entries.
Format changes to `{verb, code, payload}` dicts. The `remote.py`
entity moves from REM to FAN. Standard modes go through
`climate.set_fan_mode` → `device.set_fan_mode()` (PR 546). Only
non-standard commands (22F7 bypass, 22B0 calendar) stay in `_commands`.
See `phase3b_fan_commands_design.md`.
...
**Migration path:** Phase 3a → 3b migration moves `_commands` from REM
entries to FAN entries (see `phase3b_fan_commands_design.md`). This
does NOT wait for issue 530.
...
### Phase 3b: ramses_rf Builder alignment (after ramses_rf Phase 3 ships)

**Depends on ramses_rf issue 530 Phase 3 landing.**

| Step | Description | Status |
|------|-------------|--------|
| 3b.1 | Coordinate with PWhite-Eng on strategy key names | TODO |
| 3b.2 | Map `_class` to Builder strategy names (or deprecate) | TODO |
| 3b.3 | Move `_commands` from REM entries to FAN entries | TODO |
| 3b.4 | Stop stripping `_` keys when ramses_rf accepts them natively | TODO |
| 3b.5 | Test with Builder: verify seeding from derived known_list | TODO |
...
| Builder strategy names don't match ramses_cc's trait keys | Coordinate with PWhite-Eng before Phase 3b. Use `_profile` or `_strategy` for Builder keys. |
| Commands on REM vs FAN confusion | Phase 3a keeps commands on REM (current behavior). Phase 3b moves to FAN after Builder. |
...
2. **Commands on REM or FAN?** wimpie70 says FAN. But `remote.py`
   entities are on REM devices. **Recommendation:** Phase 3a keeps on
   REM (no entity change). Phase 3b moves to FAN when Builder handles
   command routing.
