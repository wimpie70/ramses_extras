...
async def main() -> None:
    ...
    if accept_ok:
        ...
        try:
            ...
            check(
                "TRV removed from device_comments",
                NEW_TRV not in json.dumps(
                    schema_after_remove.get("device_comments", {})
                ),
                f"device_comments still has {NEW_TRV}",
            )
            check(
                "TRV entity removed",
    ...
    # Verify the sanitisation was persisted (not just in-memory)
    check(
        "Invalid main_tcs persisted to config entry",
        "04:999999" not in json.dumps(schema_after_sanitise),
        f"config entry still references 04:999999",
    )
