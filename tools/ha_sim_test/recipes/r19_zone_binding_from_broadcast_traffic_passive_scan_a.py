...
class R19ZoneBindingFromBroadcastTrafficPassiveScanA(Recipe):
    ...
    async def run(self, ctx: RecipeContext) -> None:
        ...
        # ── Part 2: Invalid zone indices rejected (merged from R19b) ─
        ctx.log_section("Recipe 19: Invalid zone indices (>0B) are rejected")
        try:
            call_service(
                ctx.token,
                "ramses_extras",
                "device_simulator_inject_message",
                {
                    "source_id": "04:20000C",
                    "code": "30C9",
                    "payload": "0C0708",  # zone 0C invalid
                    "verb": "I",
                },
            )
        except RuntimeError:
            pass
        ctx.wait(2, "for invalid 0C inject")
        schema_post_invalid = get_schema_retry()
        ctl_post_invalid = schema_post_invalid.get(CTL, {})
        zones_post_invalid = (
            ctl_post_invalid.get("zones", {})
            if isinstance(ctl_post_invalid, dict)
            else {}
        )
        ctx.check(
            "Invalid zone 0C not created in schema",
            "0C" not in zones_post_invalid,
            f"zones={list(zones_post_invalid.keys())}",
        )

        # ── Part 3: 18: HGI tracked without zone bindings (merged from R19c) ──
        ctx.log_section("Recipe 19: 18: (HGI) devices tracked without zone bindings")
        hgi_in_schema = HGI in schema_post_invalid or any(
            k.startswith("18:") for k in schema_post_invalid
        )
        ctx.check(
            "18: device in schema",
            hgi_in_schema,
            f"keys={list(schema_post_invalid.keys())}",
        )
        for z_idx, z_data in zones_post_invalid.items():
            if isinstance(z_data, dict):
                sensor_id = z_data.get("sensor", "")
                ctx.check(
                    f"18: device not a zone sensor in zone {z_idx}",
                    not str(sensor_id).startswith("18:"),
                    f"sensor={sensor_id}",
                )
