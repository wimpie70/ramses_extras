"""Recipe R82: Config entry migration v1→v3 (0.56-era to current).

Tests the full ``async_migrate_entry`` path from config entry version 1
to version 3.  A v1 entry has:

- ``packet_log`` with deprecated ``file_name`` and ``rotate_backups`` keys
- ``ramses_rf`` with deprecated ``use_database`` and ``database_file`` keys
...
- Minimal schema (just ``orphans_hvac``)

The migration should:
1. v1→v2: Remove deprecated ``packet_log`` and ``ramses_rf`` keys
2. v2→v3: Merge ``known_list`` traits into schema as ``_``-prefixed keys,
   drop ``known_list`` and ``enforce_known_list``, bump version to 3

Verifies that after restart:
- Config entry version is 3
- ``known_list`` is gone from options
- ``enforce_known_list`` is gone from ``ramses_rf``
- Deprecated ``packet_log`` keys are removed
- Deprecated ``ramses_rf`` keys are removed
- Schema has ``_``-prefixed traits from the old ``known_list``
- A v2 backup was saved to ``.storage/ramses_cc_migration_v2_backup``
- ramses_cc loads successfully and entities are created
"""
...
def _build_v1_options(hgi_id: str) -> dict:
    """Build a v1-era config entry options dict.

    Mimics what a real 0.56-era installation would have:
    - known_list with legacy trait keys (class, alias, faked, bound, scheme)
    - enforce_known_list in ramses_rf
    - deprecated packet_log keys (file_name, rotate_backups)
    - deprecated ramses_rf keys (use_database, database_file)
    - minimal schema (just orphans_hvac, no _ traits)
    """
    return {
        "advanced_features": {
            "auto_notify": True,
        ...
        "packet_log": {
            "file_name": "/config/ramses_log.db",
            "rotate_backups": 7,
        },
        "ramses_rf": {
            "use_database": True,
            "database_file": "/config/ramses.db",
...
class R80ConfigEntryMigrationV1ToV3(Recipe):
    id = "R82"
    seq = 820
    title = "Config entry migration v1→v3 (0.56-era to current)"

    async def run(self, ctx: RecipeContext) -> None:
        ctx.log_section("Recipe 82: Config entry migration v1→v3")

        inst = get_current_instance()
        hgi_id = inst.hgi_id

        # ── Step 1: Stop HA and inject a v1 config entry ────────────
        print(f"  Stopping {inst.name} to inject v1 config entry...")
        ...
        # Replace with v1-era options
        cc_entry["options"] = _build_v1_options(hgi_id)
        cc_entry["version"] = 1
        data["data"]["entries"][cc_idx] = cc_entry

        # Also delete .storage/ramses_cc (client state cache) so the
        # migration runs clean (no cached schema to interfere)
        storage_path = f"{inst.storage_path}/ramses_cc"
        if os.path.exists(storage_path):
            os.remove(storage_path)

        # Delete any existing v2 backup so we can verify a fresh one is created
        ...
        ctx.check(
            "v1 config entry injected (version=1, known_list, deprecated keys)",
        ...
        # Read the migrated config entry
        ...
        # Check 4: deprecated packet_log keys removed
        packet_log = options.get("packet_log", {})
        ctx.check(
            "packet_log: file_name removed",
            "file_name" not in packet_log,
            f"keys={list(packet_log.keys())}",
        )
        ctx.check(
            "packet_log: rotate_backups translated to packet_log_retention_days",
            "packet_log_retention_days" in packet_log
            or "rotate_backups" not in packet_log,
            f"keys={list(packet_log.keys())}",
        )

        # Check 5: deprecated ramses_rf keys removed
        ctx.check(
            "ramses_rf: use_database removed",
            "use_database" not in ramses_rf,
            f"keys={list(ramses_rf.keys())}",
        )
        ctx.check(
            "ramses_rf: database_file removed",
            "database_file" not in ramses_rf,
            f"keys={list(ramses_rf.keys())}",
        )

        # Check 6: schema has _-prefixed traits from known_list
        schema = options.get("schema", {})
        ctx.check(
            "Schema has FAN entry with _class=FAN",
            isinstance(schema.get(FAN), dict) and schema[FAN].get("_class") == "FAN",
            f"FAN entry={schema.get(FAN)}",
        )
        ctx.check(
            "Schema has FAN _bound from known_list",
            isinstance(schema.get(FAN), dict) and schema[FAN].get("_bound") == REM,
            f"_bound={schema.get(FAN, {}).get('_bound')}",
        )
        ctx.check(
            "Schema has FAN _scheme from known_list",
            isinstance(schema.get(FAN), dict) and schema[FAN].get("_scheme") == "itho",
            f"_scheme={schema.get(FAN, {}).get('_scheme')}",
        )
        ctx.check(
            "Schema has CTL _alias from known_list",
            isinstance(schema.get(CTL), dict)
            and schema[CTL].get("_alias") == "Living Room",
            f"_alias={schema.get(CTL, {}).get('_alias')}",
        )
        ctx.check(
            "Schema has DHW _faked from known_list",
            isinstance(schema.get(DHW), dict) and schema[DHW].get("_faked") is True,
            f"_faked={schema.get(DHW, {}).get('_faked')}",
        )
        ctx.check(
            "Schema has DHW _class from known_list",
            isinstance(schema.get(DHW), dict) and schema[DHW].get("_class") == "DHW",
            f"_class={schema.get(DHW, {}).get('_class')}",
        )
        ctx.check(
            "Schema has HGI entry with _class=HGI",
        ...
        if backup_result.returncode == 0:
            ...
            ctx.check(
                "v2 backup has known_list",
                "known_list" in backup_data.get("options", {}),
                f"keys={list(backup_data.get('options', {}).keys())}",
            )

        # Check 8: ramses_cc loaded and entities exist
        ctx.check(
        ...
        # Wait for schema to be populated (scan engine may need a moment)
        ...
        # Check 9: entities were created (at least the CTL)
