...
class R58Phase4KnownListRemovalEnforceAlwaysOn(Recipe):
    ...
    async def run(self, ctx: RecipeContext) -> None:
        ...
        # ── Part 2: _cleanup_stale_known_list strips stale keys (merged from R59) ──
        ctx.log_section("Recipe 58: _cleanup_stale_known_list strips stale keys")
        inst_name = get_current_instance().name
        # Stop container, inject stale keys, start container to trigger cleanup
        subprocess.run(["docker", "stop", inst_name], capture_output=True)
        options_r58 = dict(cc_entry.get("options", {}))
        options_r58["known_list"] = {CTL: {"class": "CTL"}}
        ramses_rf_r58 = dict(options_r58.get("ramses_rf", {}))
        ramses_rf_r58["enforce_known_list"] = False
        options_r58["ramses_rf"] = ramses_rf_r58
        cc_entry["options"] = options_r58
        data_entries = data["data"]["entries"]
        for idx, entry in enumerate(data_entries):
            if entry.get("domain") == "ramses_cc":
                data_entries[idx] = cc_entry
                break
        ...
        subprocess.run(
            [
                ...
                "cp",
                tmp_path_r58,
                f"{inst_name}:/config/.storage/core.config_entries",
            ],
            capture_output=True,
        )
        os.unlink(tmp_path_r58)

        subprocess.run(["docker", "start", inst_name], capture_output=True)
        from ..helpers import wait_for_ha_ready

        wait_for_ha_ready(timeout=30)
        ctx.log_monitor.reset_baseline()
        ctx.refresh_token()
        ctx.wait(5, "for ramses_cc cleanup on startup")

        res_r58 = subprocess.run(
            [
                "docker",
                "exec",
                inst_name,
        ...
        if res_r58.returncode == 0:
            data_r58 = json.loads(res_r58.stdout)
            cc_entry_r58 = next(
                (e for e in data_r58["data"]["entries"] if e["domain"] == "ramses_cc"),
                None,
            )
            if cc_entry_r58:
                opts_r58 = cc_entry_r58.get("options", {})
                kl_keys = list(opts_r58.get("known_list", {}).keys())
                enforce_val = opts_r58.get("ramses_rf", {}).get("enforce_known_list")
                ctx.check(
                    "stale known_list stripped by _cleanup_stale_known_list",
                    "known_list" not in opts_r58,
                    f"known_list still present: {kl_keys}",
                )
                ctx.check(
                    "stale enforce_known_list stripped",
                    "enforce_known_list" not in opts_r58.get("ramses_rf", {}),
                    f"enforce_known_list still present: {enforce_val}",
                )
