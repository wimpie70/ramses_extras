...
class TestDeviceFeatureMatrix:
    ...
    def test_matrix_state_management(self):
        ...
        new_matrix.restore_device_feature_matrix_state(state)
        assert new_matrix.is_feature_enabled_for_device("feature1", "device1") is True
...
class TestEntityCreationValidation:
    ...
    def test_entity_creation_only_for_enabled_feature_and_device(self):
        """Test that entities are only created when BOTH feature and
        device are enabled."""
...
class TestEntityCreationPerformance:
    ...
    def test_matrix_memory_usage(self):
        ...
        # Verify restoration
        combinations = new_matrix.get_all_enabled_combinations()
        assert len(combinations) == 100000  # 1000 devices × 100 features
